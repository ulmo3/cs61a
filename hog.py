...
def take_turn(num_rolls, opponent_score, dice=six_sided):
    ...
    if num_rolls == 0:
        ...
        return roll_dice(num_rolls, dice) #why cannot use dice=six_sided?
    # END PROBLEM 3
...
def perfect_square(score):
    square_num = 1
    while square_num <= sqrt(score):
        if square_num * square_num == score:
            return True
        square_num += 1
    return False
...
def is_always_roll(strategy, goal=GOAL):
    ...
    """
    # BEGIN PROBLEM 7
    roll_what = strategy(0,0)
    for i in range(100):
        for j in range(100):
            if roll_what != strategy(i, j):
                return False
            roll_what = strategy(i, j)
    return True
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    while roll_num <= 10:
        ...
        tmp = averaged_dice(roll_num, dice)
        if tmp > max_score:
            max_score = tmp
            max_scoring_num_rolls = roll_num
        elif tmp == max_score:
            if roll_num <= max_scoring_num_rolls:
                max_scoring_num_rolls = roll_num
        roll_num += 1
